#!/usr/bin/env python3
"""
fetch_range.py — OGUN_CSV Edition
Baseball Savant Data Pipeline: Fetch → Clean → Write CSV

No database. Outputs one CSV per month to data/YYYY_MM.csv.
Duplicate prevention via play_id — safe to re-run on existing range.

Usage:
    python fetch_range.py
    # Edit START_DATE / END_DATE below before running.
"""
...
# ── Date Range ─────────────────────────────────────────────────────────────────
START_DATE = date(2025, 3, 27)   # Opening Day 2025
...
NUMERIC_COLS = [
    ...
    "hc_x_ft", "hc_y_ft", "launch_speed", "launch_angle",
    "game_number",
]
ROUND_2 = [
    "start_speed", "end_speed", "sz_top", "sz_bot",
...
def fetch_schedule(d: date) -> list[dict]:
    """Return Final games for date d. Logs non-Final games to edge_cases.log."""
    ...
    if not isinstance(data, dict):
        log.info("NO_GAMES  date=%s  (non-dict response)", d)
        return []
...
def fetch_game_pitches(game_meta: dict) -> list[dict]:
    """Fetch /gf for one game. Returns flat list of pitch-row dicts."""
...
def clean_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing:
        log.warning("  Whitelist columns absent: %s", missing)
    ...
    if "is_bip_out" in df.columns:
        df["is_bip_out"] = df["is_bip_out"].map({"Y": True, "N": False, True: True, False: False})
...
def csv_path_for_month(y: int, m: int) -> Path:
    return DATA_DIR / f"{y}_{m:02d}.csv"


def write_csv(df: pd.DataFrame) -> None:
    """
    Write df to monthly CSV files (one per calendar month).
    Merges with any existing CSV — deduplicates on play_id — then overwrites.
    """
    if df.empty:
        log.info("  No rows to write.")
        return

    # Group incoming data by month
    ...
    for ym, chunk in df.groupby("_ym"):
        ...
        if out_path.exists():
            ...
            skipped = len(combined) - (len(combined) - before + len(chunk))
            new_rows = len(combined) - before
            log.info("  %s — merged: %d existing + %d incoming → %d new rows written",
                     out_path.name, before, len(chunk), new_rows)
        else:
            combined = chunk
            log.info("  %s — new file, writing %d rows", out_path.name, len(combined))

        combined.to_csv(out_path, index=False, encoding="utf-8")
        log.info("  Wrote %s (%d rows total)", out_path, len(combined))
...
def main():
    log.info("=" * 60)
    log.info("OGUN_CSV Pipeline  |  %s → %s", START_DATE, END_DATE)
    log.info("=" * 60)

    # Phase 1 — Fetch schedules
    ...
    # Phase 3 — Sort
    log.info("PHASE 3 — Sorting...")
    df = sort_dataframe(df)
    log.info("Sort complete. Final shape: %s", df.shape)

    # Phase 4 — Write CSV
    log.info("PHASE 4 — Writing CSV(s) to %s ...", DATA_DIR)
    write_csv(df)
